...
int thermodynamics_init(
  ...
  if(pth->annihilation >0 || pth->decay_fraction > 0 || pth->PBH_accreting_mass > 0 || pth->PBH_evaporating_mass > 0){
    fprintf(stdout,"Annihilation parameter (pth->annihilation) = %.2e in line 333 of thermodynamics.c. \n",pth->annihilation);
...
int thermodynamics_annihilation_coefficients_interpolate(
  ...
  class_call(array_interpolate_spline(pth->annihil_coef_xe,
    ...
    if(pth->thermodynamics_verbose>15){
      // fprintf(stdout,"z = %.2e, chi_heat = %.2e interpolated from annihil_coef_heat in line 1498 of thermodynamics. \n",xe_or_z,pth->chi_heat);
      // fprintf(stdout,"z = %.2e, chi_lya = %.2e interpolated from annihil_coef_lya in line 1512 of thermodynamics. \n",xe_or_z,pth->chi_lya);
      // fprintf(stdout,"z = %.2e, chi_ionH = %.2e interpolated from annihil_coef_ionH in line 1498 of thermodynamics. \n",xe_or_z,pth->chi_ionH);
      // fprintf(stdout,"z = %.2e, chi_ionHe = %.2e interpolated from annihil_coef_ionHe in line 1540 of thermodynamics. \n",xe_or_z,pth->chi_ionHe);
      // fprintf(stdout,"z = %.2e, chi_lowE = %.2e interpolated from annihil_coef_lowE in line 1554 of thermodynamics. \n",xe_or_z,pth->chi_lowE);
    }
...
int thermodynamics_annihilation_f_eff_interpolate(
  ...
  fprintf(stdout,"z = %.2e, f_eff = %.2e interpolated from annihil_f_eff in line 1706 of thermodynamics. \n",z,preco->f_eff);
...
int thermodynamics_DM_annihilation_energy_injection(
  ...
  // Boost_factor = preco->annihilation_f_halo*erfc((1+z)/(1+preco->annihilation_z_halo))/pow(1+z,3);
    // changed to remove below multiplication by the boost factor, to correspond with removal of boost in the denominator of f(z) in DarkAges
    // note boost is still used in annihilation case in DarkAges in the *numerator* of f(z) - this doesn't remove it entirely
    Boost_factor = 0;
  }
  else Boost_factor = 0;

  // fprintf(stdout,"Annihilation parameter (preco->annihilation) = %.2e in line 1756 of thermodynamics.c. \n",preco->annihilation);
  *energy_rate = pow(rho_cdm_today,2)/_c_/_c_*(pow((1.+z),6)*preco->annihilation)*(1+Boost_factor);
  ...
  // fprintf(stdout, "*energy_rate %e in thermodynamics_DM_annihilation_energy_injection\n",*energy_rate );
    ...
    if (preco->has_on_the_spot == _FALSE_) {
      ...
      // // /***********************************************************************************************************************/
            ...
            class_call(thermodynamics_onthespot_energy_injection(ppr,pba,preco,z,&result,error_message),
                      error_message,
                      error_message);
            // fprintf(stdout, "energy_rate %e in thermodynamics_energy_injection \n", result);
      }
...
int thermodynamics_reionization(
  ...
  if((pth->reio_parametrization == reio_stars_sfr_source_term)){
    ...
    //Helium reionization is still a tanh, to be improved
  ...
  for (i=preio->rt_size-1; i >0 ; i--) {
    ...
    dTdz_CMB = - 2.*mu/_m_e_*4.*pvecback[pba->index_bg_rho_g]/3./pvecback[pba->index_bg_rho_b]*opacity*
      ...
      if((pth->annihilation != 0 || pth->decay_fraction != 0 || pth->PBH_accreting_mass != 0 || pth->PBH_evaporating_mass != 0)){
            ...
            // fprintf(stdout,"Annihilation parameter (pth->annihilation) = %.2e in line 3805 of thermodynamics.c. \n",pth->annihilation);
            /** - --> derivative of baryon temperature */
      ...
      if(pth->thermodynamics_verbose == 18){
        fprintf(stdout, "TRS1 z %e pth->chi_ionH %e pth->chi_ionHe %e pth->chi_Lya %e\n", z,pth->chi_ionH,pth->chi_ionHe,pth->chi_lya);
        fprintf(stdout, "TRS2 z %e dTdz %e dTdz_adia %e dTdz_CMB %e dTdz_DM %e dTdz_stars %e xe %e energy_rate %e chi_heat %e\n", z,dTdz,dTdz_adia,dTdz_CMB,dTdz_DM,dTdz_stars,preio->reionization_table[i*preio->re_size+preio->index_re_xe],energy_rate,chi_heat);
...
int thermodynamics_derivs_with_recfast(
  ...
  class_call(background_at_tau(pba,
   ...
   if((pth->annihilation!=0 || pth->decay_fraction!=0 || pth->PBH_accreting_mass!=0 || pth->PBH_evaporating_mass != 0)){
     ...
     // fprintf(stdout,"Annihilation parameter (pth->annihilation) = %.2e in line 5006 of thermodynamics.c, in RECFAST. \n",pth->annihilation);
 ...
 // fprintf(stdout,"%e      %e     %e      %e      %e    \n", x,pth->chi_heat,pth->chi_lya, pth->chi_ionH,pth->chi_ionHe,pth->chi_lowE);
  ...
  if (x_H > ppr->recfast_x_H0_trigger)
    ...
    if(preco->annihilation > 0 || preco->decay_fraction > 0 || preco->PBH_accreting_mass > 0 || preco->PBH_evaporating_mass > 0){
      // fprintf(stdout,"Annihilation parameter (preco->annihilation) = %.2e in line 5124 of thermodynamics.c, in RECFAST. \n",preco->annihilation);
      if (x < 1.){
        ...
        if(pth->energy_repart_coefficient==no_factorization){
          ...
          chi_ionHe = pth->chi_ionHe;
          chi_lya = pth->chi_lya;
          // fprintf(stdout, "z, chi_ionH, chi_lya, pth->chi_heat, sum on line 5143 in thermodynamics.c\n");
      ...
      // if(pth->thermodynamics_verbose>10){
      //   if(0.1>abs(2000-z) || 0.1>abs(1000-z) || 0.1>abs(z-500) || 0.1>abs(z-100) || 0.1>abs(z-50) || 0.1>abs(z-10)){
      //     fprintf(stdout, "z %e chi_ionH %e chi_ionHe %e chi_lya %e sum %e \n", z, chi_ionH , chi_ionHe, chi_lya, chi_ionH+chi_ionHe+chi_lya);  
      //   }
      // }
    }
    ...
    if (x_H < ppr->recfast_x_H0_trigger2) {
      ...
      // if(z == 0.0 || z == 21.0 || z == 52.0 || z == 106.0 || z == 215.0 || z == 518.0 || z == 1050.0 || z == 2110.0){
      //   fprintf(stdout,"z = %.2e, energy_rate = %.2e in line 5186 of thermodynamics.c, in RECFAST x_e calc. \n",z,energy_rate);  
      // }
    ...
    //fprintf(stdout,"%e  %e  %e  %e\n",z,Tmat,K*_Lambda_*n,K*Rup*n);
      ...
      if(pth->thermodynamics_verbose == 18){
        fprintf(stdout, "Rxe z %e dy[0] %e x %e x_H %e energy_rate %e chi_ionH %e chi_ionHe %e chi_lya %e\n", z, dy[0], x, x_H, energy_rate, chi_ionH, chi_ionHe, chi_lya);
  ...
  if (timeTh < preco->H_frac*timeH) {
    ...
    if(pth->annihilation >0 || pth->decay_fraction > 0 || pth->PBH_accreting_mass > 0 || pth->PBH_evaporating_mass > 0){
      // fprintf(stdout,"Annihilation parameter (pth->annihilation) = %.2e in line 5245 of thermodynamics.c, in RECFAST. \n",pth->annihilation);
      if (x < 1.){
        ...
        if(pth->energy_repart_coefficient==no_factorization){
          class_call(thermodynamics_annihilation_coefficients_interpolate(ppr,pba,pth,z),
                        ...
                        error_message);
            chi_heat = pth->chi_heat;
            // fprintf(stdout,"chi_heat = %.2e in line 5284 of thermodynamics.c, in RECFAST. \n",chi_heat);
        }
        /* old approximation from Chen and Kamionkowski */
        if(pth->energy_repart_coefficient==SSCK){
            chi_heat = (1.+2.*x)/3.;
    ...
    // if(pth->thermodynamics_verbose==7){
    //   if(z == 0.0 || z == 21.0 || z == 52.0 || z == 106.0 || z == 215.0 || z == 518.0 || z == 1050.0 || z == 2110.0){
    //     fprintf(stdout,"z = %e, chi_heat = %e in line 5300 of thermodynamics.c, in RECFAST. \n",z,chi_heat);  
    //   }
    // }
    ...
    if(pth->thermodynamics_verbose == 18){
      ...
      // z, chi_heat, energy_rate, x_e, dTdz_DM
      fprintf(stdout, "RTM z %e dy[2] %e dTdz_CMB %e dTdz_adia %e dTdz_DM %e dTdz_stars %e energy_rate %e chi_heat %e x %e\n",z,dy[2],dTdz_CMB,dTdz_adia,dTdz_DM,dTdz_stars,energy_rate,chi_heat,x);

      // if(z == 0.0 || z == 21.0 || z == 52.0 || z == 106.0 || z == 215.0 || z == 518.0 || z == 1050.0 || z == 2110.0){
      //     // fprintf(stdout, "z %e chi_heat %e chi_lya %e chi_ionH %e chi_ionHe %e sum %e \n", z, chi_heat, chi_lya, chi_ionH , chi_ionHe, chi_ionH+chi_ionHe+chi_lya);  
      //   fprintf(stdout, "z = %e, chi_heat = %e, energy_rate = %e, dTdz_DM = %e \n", z, chi_heat, energy_rate, dTdz_DM);
      //   }
